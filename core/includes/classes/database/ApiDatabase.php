...
class ApiDatabase
{
    ...
    {
        $id = $this->conn->MySQL->SQLValue($id, MySQL::SQLVALUE_NUMBER);
    ...
    public function expense_get($expId)
    {
        ...
        $expId = $this->conn->MySQL->SQLValue($expId, MySQL::SQLVALUE_NUMBER);
                                 ...
                                 $startRows = 0, $limitRows = 0, $countOnly = false)
    {
        ...
        $start = $this->conn->MySQL->SQLValue($start, MySQL::SQLVALUE_NUMBER);
        $end   = $this->conn->MySQL->SQLValue($end, MySQL::SQLVALUE_NUMBER);
    ...
    function expense_create(Array $data)
    {
        ...
        if (isset($data ['timestamp'])) {
            $values ['timestamp'] = $conn->MySQL->SQLValue($data['timestamp'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['userID'])) {
            $values ['userID'] = $conn->MySQL->SQLValue($data['userID'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['projectID'])) {
            $values ['projectID'] = $conn->MySQL->SQLValue($data['projectID'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['designation'])) {
            $values ['designation'] = $conn->MySQL->SQLValue($data['designation']);
        }
        if (isset($data ['comment'])) {
            $values ['comment'] = $conn->MySQL->SQLValue($data['comment']);
        }
        if (isset($data ['commentType'])) {
            $values ['commentType'] = $conn->MySQL->SQLValue($data['commentType'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['refundable'])) {
            $values ['refundable'] = $conn->MySQL->SQLValue($data['refundable'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['cleared'])) {
            $values ['cleared'] = $conn->MySQL->SQLValue($data['cleared'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['multiplier'])) {
            $values ['multiplier'] = $conn->MySQL->SQLValue($data['multiplier'], MySQL::SQLVALUE_NUMBER);
        }
        if (isset($data ['value'])) {
            $values ['value'] = $conn->MySQL->SQLValue($data['value'], MySQL::SQLVALUE_NUMBER);
        }
    ...
    function expense_edit($id, Array $data)
    {
        ...
        $values ['projectID']   = $conn->MySQL->SQLValue($new_array ['projectID'], MySQL::SQLVALUE_NUMBER);
        $values ['designation'] = $conn->MySQL->SQLValue($new_array ['designation']);
        $values ['comment']     = $conn->MySQL->SQLValue($new_array ['comment']);
        $values ['commentType'] = $conn->MySQL->SQLValue($new_array ['commentType'], MySQL::SQLVALUE_NUMBER);
        $values ['timestamp']   = $conn->MySQL->SQLValue($new_array ['timestamp'], MySQL::SQLVALUE_NUMBER);
        $values ['multiplier']  = $conn->MySQL->SQLValue($new_array ['multiplier'], MySQL::SQLVALUE_NUMBER);
        $values ['value']       = $conn->MySQL->SQLValue($new_array ['value'], MySQL::SQLVALUE_NUMBER);
        $values ['refundable']  = $conn->MySQL->SQLValue($new_array ['refundable'], MySQL::SQLVALUE_NUMBER);
        $values ['cleared']     = $conn->MySQL->SQLValue($new_array ['cleared'], MySQL::SQLVALUE_NUMBER);

        $filter ['expenseID'] = $conn->MySQL->SQLValue($id, MySQL::SQLVALUE_NUMBER);
        $table                = $this->getExpenseTable();
