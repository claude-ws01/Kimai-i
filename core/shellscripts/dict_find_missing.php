...
/*  RUN EXAMPLE
    # use <en.php> as reference, check <de.php> and put results in <de_new.php>
    php dict_find_missing.php en de de_new

    BE AWARE
    Resulting file DOES NOT contain the "// REVIEW" notes contained in the checked file.
    So... better complete the previous review before re-running <dict_find_missing.php>
*/
// Get command line arguments or set default values
$keyPath    = !empty($argv[1]) ? '../language/' . trim($argv[1]) . '.php' : '../language/en.php';
$valuePath  = !empty($argv[2]) ? '../language/' . trim($argv[2]) . '.php' : '../language/es.php';
$targetPath = !empty($argv[3]) ? '../language/' . trim($argv[3]) . '.php' : '../language/new_' . date('Ymd\THis') . '.php';
