...
switch ($axAction) {
    ...
    case "add_edit_timeSheetEntry":
        ...
        if ($id) {
            ...
            if ($timesheet_entry['user_id'] == $kga['user']['user_id']) {
                // the user's own entry
                if (!$database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__own_entry__edit')) {
                    break;
                }
            }
            else {
                if (count(array_intersect(
                              $database->user_get_group_ids($kga['user']['user_id']),
                              $database->user_get_group_ids($timesheet_entry['user_id'])
                          )) != 0
                ) {
                    // same group as the entry's user
                    if (!$database->checkMembershipPermission($kga['user']['user_id'], $database->user_get_group_ids($timesheet_entry['user_id']), 'ki_timesheets__other_entry__own_group__edit')) {
                        break;
                    }
                }
                else {
                    if (!$database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__other_entry__other_group__edit')) {
                        break;
                    }
                }
            }
            ...
            if ($database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__other_entry__other_group__edit')) {
                ...
                if ($database->checkMembershipPermission($kga['user']['user_id'], $database->user_get_group_ids($kga['user']['user_id']), 'ki_timesheets__other_entry__own_group__edit')) {
            ...
            $view->showRate   = $database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__edit_rates');
            ...
            $view->status_id = $kga['conf']['defaultStatusID'];


            $users = array();
            if ($database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__other_entry__other_group__add')) {
                $users = makeSelectBox("allUser", any_get_group_ids());
            }
            else {
                if ($database->checkMembershipPermission($kga['user']['user_id'], $database->user_get_group_ids($kga['user']['user_id']), 'ki_timesheets__other_entry__own_group__add')) {
                    $users = makeSelectBox("sameGroupUser", any_get_group_ids());
                }
            }
            if ($database->global_role_allows(any_get_global_role_id(), 'ki_timesheets__own_entry__add')) {
                $users[$kga['user']['user_id']] = $kga['user']['name'];
            }
