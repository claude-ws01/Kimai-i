...
class BasePDF extends TCPDF
{
    ...
    public function summarize($orderedExportData)
    {
            ...
            foreach ($project_ids as $project_id) {
                foreach ($customer[$project_id] as $row) {
                    ...
                    if ($row['type'] == 'timesheet') {
                        if (isset($timesheet_summary[$row['activity_id']])) {
                            $timesheet_summary[$row['activity_id']]['time'] += ($GLOBALS['kga']['conf']['exact_sums'] == 1) ? $row['duration'] / 3600 : $row['decimal_duration']; //Sekunden
                            $timesheet_summary[$row['activity_id']]['wage'] += ($GLOBALS['kga']['conf']['exact_sums'] == 1) ? $row['wage_decimal'] : $row['wage']; //Euro
                        }
                        else {
                            $timesheet_summary[$row['activity_id']]['name'] = html_entity_decode($row['activity_name']);
                            $timesheet_summary[$row['activity_id']]['time'] = ($GLOBALS['kga']['conf']['exact_sums'] == 1) ? $row['duration'] / 3600 : $row['decimal_duration'];
                            $timesheet_summary[$row['activity_id']]['wage'] = ($GLOBALS['kga']['conf']['exact_sums'] == 1) ? $row['wage_decimal'] : $row['wage'];
                        }
                    }
                    else {
        ...
        return array_merge($timesheet_summary, $expenseSummary);
    }
