...
class MYPDF extends BasePDF
{
    ...
    public function ColoredTable($header, $data)
    {
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($w[1], 6, $row['customer_name'] . ' - ' . $row['activity_name'], 'LR', 0, 'L', $fill);

            if (isset($this->columns['dec_time'])) {
                $this->Cell($w[2], 6, $this->timespan(isset($row['decimal_duration']) ? $row['decimal_duration'] : 0), 'LR', 0, 'R', $fill);
            }
            if (isset($this->columns['wage'])) {
            ...
            $fill = !$fill;
            $moneySum += $row['wage'];
            $timeSum += $row['decimal_duration'] == -1 ? 0 : $row['decimal_duration'];
