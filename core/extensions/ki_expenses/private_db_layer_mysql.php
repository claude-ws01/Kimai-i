...
function expense_create($userID, array $data)
{
    ...
    $values ['projectID']   = $database->MySQL->SQLValue($data ['projectID'], MySQL::SQLVALUE_NUMBER);
    $values ['designation'] = $database->MySQL->SQLValue($data ['designation']);
    $values ['comment']     = $database->MySQL->SQLValue($data ['comment']);
    $values ['commentType'] = $database->MySQL->SQLValue($data ['commentType'], MySQL::SQLVALUE_NUMBER);
    $values ['timestamp']   = $database->MySQL->SQLValue($data ['timestamp'], MySQL::SQLVALUE_NUMBER);
    $values ['multiplier']  = $database->MySQL->SQLValue($data ['multiplier'], MySQL::SQLVALUE_NUMBER);
    $values ['value']       = $database->MySQL->SQLValue($data ['value'], MySQL::SQLVALUE_NUMBER);
    $values ['userID']      = $database->MySQL->SQLValue($userID, MySQL::SQLVALUE_NUMBER);
    $values ['refundable']  = $database->MySQL->SQLValue($data ['refundable'], MySQL::SQLVALUE_NUMBER);

    $table  = $kga['server_prefix'] . "expenses";
    $result = $database->MySQL->InsertRow($table, $values);

    if (!$result) {
        Logger::logfile('expense_create: ' . $database->MySQL->Error());
...
function expenses_widthhereClausesFromFilters($users, $customers, $projects)
{
    ...
    foreach ($users as $i => $user) {
        $users[$i] = $database->MySQL->SQLValue($user, MySQL::SQLVALUE_NUMBER);
    }

    foreach ($customers as $i => $customer) {
        $customers[$i] = $database->MySQL->SQLValue($customer, MySQL::SQLVALUE_NUMBER);
    }

    foreach ($projects as $i => $project) {
        $projects[$i] = $database->MySQL->SQLValue($project, MySQL::SQLVALUE_NUMBER);
    }
...
function get_expenses($start, $end, $users = null, $customers = null, $projects = null, $limit = false, $reverse_order = false, $filter_refundable = -1, $filterCleared = null)
{
    ...
    $start = $database->MySQL->SQLValue($start, MySQL::SQLVALUE_NUMBER);
    $end   = $database->MySQL->SQLValue($end, MySQL::SQLVALUE_NUMBER);
    $limit = $database->MySQL->SQLValue($limit, MySQL::SQLVALUE_NUMBER);
    ...
    $database->MySQL->MoveFirst();
    while (!$database->MySQL->EndOfSeek()) {
        $row                    = $database->MySQL->Row();
...
function get_expense($id)
{
    ...
    $id = $database->MySQL->SQLValue($id, MySQL::SQLVALUE_NUMBER);
    $p  = $kga['server_prefix'];
    ...
    return $database->MySQL->RowArray(0, MYSQL_ASSOC);
}
 ...
 *
 * @param int    $expenseID expenseID of the record
 *
 * @global array $kga       kimai-global-array
...
function expense_get($expenseID)
{
    ...
    $expenseID = $database->MySQL->SQLValue($expenseID, MySQL::SQLVALUE_NUMBER);

    if ($expenseID) {
        $result = $database->MySQL->Query("SELECT * FROM ${p}expenses WHERE expenseID = " . $expenseID);
    }
    else {
        $result = $database->MySQL->Query("SELECT * FROM ${p}expenses WHERE userID = " . $kga['user']['userID'] . " ORDER BY expenseID DESC LIMIT 1");
    }
...
function expense_edit($id, array $data)
{
    ...
    $values ['projectID']   = $database->MySQL->SQLValue($new_array ['projectID'], MySQL::SQLVALUE_NUMBER);
    $values ['designation'] = $database->MySQL->SQLValue($new_array ['designation']);
    $values ['comment']     = $database->MySQL->SQLValue($new_array ['comment']);
    $values ['commentType'] = $database->MySQL->SQLValue($new_array ['commentType'], MySQL::SQLVALUE_NUMBER);
    $values ['timestamp']   = $database->MySQL->SQLValue($new_array ['timestamp'], MySQL::SQLVALUE_NUMBER);
    $values ['multiplier']  = $database->MySQL->SQLValue($new_array ['multiplier'], MySQL::SQLVALUE_NUMBER);
    $values ['value']       = $database->MySQL->SQLValue($new_array ['value'], MySQL::SQLVALUE_NUMBER);
    $values ['refundable']  = $database->MySQL->SQLValue($new_array ['refundable'], MySQL::SQLVALUE_NUMBER);
...
function expenses_by_user($start, $end, $users = null, $customers = null, $projects = null)
{
    ...
    $query = "SELECT SUM(value*multiplier) as expenses, userID
             ...
             Join ${p}users USING(userID) " . (count($whereClauses) > 0 ? " WHERE "
    ...
    $rows = $database->MySQL->RecordsArray(MYSQL_ASSOC);
    if (!$rows) {
        return array();
    }
...
function expenses_by_customer($start, $end, $users = null, $customers = null, $projects = null)
{
    ...
    $query = "SELECT SUM(value*multiplier) as expenses, customerID FROM ${p}expenses
            ...
            Left Join ${p}customers USING(customerID) " . (count($whereClauses) > 0 ? " WHERE "
            : " ") . implode(" AND ", $whereClauses) .
        " GROUP BY customerID;";
