...
function expense_extension_set_TableWidths() {
    var scr,
        expenses_table = $("#exptable > table");

    expense_extension_get_dimensions();

    // width of expenses table depending on scrollbar or not
    ($("#expenses").innerHeight() - expenses_table.outerHeight() > 0) ? scr = 0 : scr = scroller_width;

    // set table widths
    $("#expenses_head > table").css("width", expenses_width - scr);

    // manage width of HEAD column
    if ($("div#expenses > div > table > tbody > tr > td.option").width() != null) {
        $("#expenses_head > table > tbody > tr > td.option").css("width", $("div#expenses > div > table > tbody > tr > td.option").width());
    }
    $("#expenses_head > table > tbody > tr > td.date").css("width", $("div#expenses > div > table > tbody > tr > td.date").width());
    ...
    $("#expenses_head > table > tbody > tr > td.username").css("width", $("div#expenses > div > table > tbody > tr > td.username").width());
}
